...
void BranchBuilding(float CellSize, 
		    ...
		    const bool allowSameLayerSeedMerge)
{
  ...
  int SizeA = 0;
  int SizeB = 0;
  TVector3 PosBranch_A, PosBranch_B, Pos_Diff;
  for(int t3 = 0; t3 < NBranches; t3++)
  {
	  const auto& branch_A = PrunedInitBranchCollection[t3];  //Longer to shorter range
	  SizeA = branch_A.size();
	  if(SizeA > 15)
	  {
		  for(int t4 = t3 + 1; t4 < NBranches; t4++)
		  {
			  const auto& branch_B = PrunedInitBranchCollection[t4];
			  SizeB = branch_B.size();
			  if( SizeB > 15)
			  {
				  if( SizeA > SizeB )
				  ...
				  for(int u3 = 0; u3 < SizeA; u3++)
				  {
					  PosBranch_A = cleanedHits[branch_A[u3]].first;
					  for(int u4 = 0; u4 < SizeB; u4++)
					  {
						  PosBranch_B = cleanedHits[branch_B[u4]].first;

						  Pos_Diff = PosBranch_A - PosBranch_B;

						  if(Pos_Diff.Mag() < 1.1*CellSize && abs(Pos_Diff.Z()) < 0.001 )         //Only allow Same layer Joint...
						  {
							  NJoint++;
						  }
					  }
				  ...
				  if(NJoint > 0.6*MinCluSize)
				  {
					  cout<<"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA        JJJJJJJJJJJJJJJJOOOOOOOOOOOOOOOOOOOOIIIIIIIIIIIIIIIIIIIIIINNNNNNNNNNNNNNNNNNNNNTTTTTTTTTTTTTTTT"<<endl;

					  if( !qu.connected(t3, t4) )
						  qu.unite(t3, t4);
				  }
				  // cout<<"BranchSize_A_B: "<<branch_A.size()<<" : "<<branch_B.size()<<" the minimal "<<MinCluSize<<"  with Joints "<<NJoint<<endl;
			  }
		  }
	  }
  }
...
std::vector< std::vector<int> > Arbor(std::vector<std::pair<TVector3,float> > inputHits, 
	...
	BranchBuilding(CellSize, distSeedForMerge,allowSameLayerSeedMerge);	
	BushMerging();
